VideoType = 'CASINO_SNWFLK_PL'
EnterEvent = 'chCasinoWall:enteredCasino'
ExitEvent  = 'chCasinoWall:exitedCasino'
BigWinEvent = 'chCasinoWall:bigWin'
...
AddEventHandler(EnterEvent, function()
    inCasino = true

    startCasinoThreads()
end)

AddEventHandler(ExitEvent, function()
    inCasino = false
end)

AddEventHandler(BigWinEvent, function()
    if not inCasino then
        return
    end
