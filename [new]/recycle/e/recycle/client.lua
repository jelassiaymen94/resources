	...
	for k, v in pairs(Config.Locations) do
		if Config.Locations[k].blipTrue then
			...
			if Config.BlipNamer then
				...
				AddTextComponentString("Los Santos Recycling Center")
			end
...
Citizen.CreateThread(function()
    ...
    exports['qb-target']:AddCircleZone("recycleduty", vector3(1201.99, -3267.47, 5.5), 2.0, { name="recycleduty", debugPoly=false, useZ=true, },
    { options = { { event = "recycle:dutytoggle", icon = "fas fa-hard-hat", label = "Toggle Recycling Duty", }, },
					distance = 1.5
    })-- vector3(1201.99, -3267.47, 5.5)
