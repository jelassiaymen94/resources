...
(() => {
    ...
    $(document).on('click', '.character', function(e) {
        ...
        if (selectedChar === null) {
            ...
            if ((selectedChar).data('cid') == "") {
                ...
                $("#play").fadeIn(50)
                $("#delete").fadeIn(50)
                $.post('https://multicharacter/loadSkin', JSON.stringify({cData: cDataPed}));
            }
    ...
    $(document).on('click', '#play', function(e) {
        ...
        if (selectedChar !== null) {
            if (charData !== "") {
                $.post('https://multicharacter/selectCharacter', JSON.stringify({
                    cData: $(selectedChar).data('cData')
                }));
            } else {
                $.post('https://multicharacter/register', JSON.stringify({
                    slots: slots,
                }))
            }
        }
    ...
    $(document).on('click', '#accept-delete', function(e) {
        ...
        if (selectedChar !== null) {
            if (charData !== "") {
                $.post('https://multicharacter/delete', JSON.stringify({
                    cData: $(selectedChar).data('cData')
                }))
        ...
        window.addEventListener('message', function(event) {
            switch(event.data.action) {
                ...
                case 'setupCharacters':
                    ...
                    break
                case 'loaded':
                    $.post('https://multicharacter/loaded', JSON.stringify({}))
                    break
            }
        })
        $.post('https://multicharacter/loaded', JSON.stringify({}))
