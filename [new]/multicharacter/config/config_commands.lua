...
QBCore.Commands.Add('deletecharacter', Config.Translate['cmd.help_deletecharacter'], {{ 
    ...
    if (args[1]) then
        ...
        TriggerClientEvent('multicharacter:notification', source, (Config.Translate['cmd.success_deleted_character']):format(args[1]), 5500, 'success')
    end
end, 'admin')
    ...
    if (args[1] and tonumber(args[2])) then
        MySQL.Async.fetchAll('SELECT slots FROM multichars_slots WHERE identifier = ?', {args[1]}, function(result)
            if result[1] then
                ...
                TriggerClientEvent('multicharacter:notification', source, (Config.Translate['slots_edited']):format(args[2], args[1]), 5500, 'success')
            else
                MySQL.update('INSERT INTO `multichars_slots` (`identifier`, `slots`) VALUES (?, ?)', {args[1], args[2]})
                TriggerClientEvent('multicharacter:notification', source, (Config.Translate['slots_added']):format(args[2], args[1]), 5500, 'success')
            end
        end)
    end
end, 'admin')
    ...
    if (args[1]) then
        MySQL.Async.fetchAll('SELECT slots FROM multichars_slots WHERE identifier = ?', {args[1]}, function(result)
            if result[1] then
                ...
                TriggerClientEvent('multicharacter:notification', source, (Config.Translate['slots_removed']):format(args[1]), 5500, 'success')
            end
        end)
    end
end, 'admin')
...
QBCore.Commands.Add('enablechar', Config.Translate['cmd.help_enablechar'], {{ 
    ...
    if (args[1]) then
        MySQL.update('UPDATE `players` SET `disabled` = 0 WHERE citizenid = ?', {args[1]}, function(result)
            if result > 0 then
                TriggerClientEvent('multicharacter:notification', source, Config.Translate['charenabled']:format(args[1]), 5500, 'success')
            else
                TriggerClientEvent('multicharacter:notification', source, Config.Translate['charnotfound']:format(args[1]), 5500, 'error')
            end
        end)
    end
end, 'admin')
...
QBCore.Commands.Add('disablechar', Config.Translate['cmd.help_disablechar'], {{ 
    ...
    if (args[1]) then
        MySQL.update('UPDATE `users` SET `disabled` = 1 WHERE identifier = ?', {args[1]}, function(result)
            if result > 0 then
                TriggerClientEvent('multicharacter:notification', source, Config.Translate['chardisabled']:format(args[1]), 5500, 'success')
            else
                TriggerClientEvent('multicharacter:notification', source, Config.Translate['charnotfound']:format(args[1]), 5500, 'error')
            end
        end)
