...
QBCore.Commands.Add('register', Config.Translate['cmd.help_register'], {{ 
    ...
    if (args[1]) then
        TriggerClientEvent('identity:showRegisterIdentity', args[1])
        TriggerClientEvent('multicharacter:notification', source, (Config.Translate['cmd.opened_register']):format(args[1]), 5500, 'success')
    end
end, 'admin')
...
QBCore.Functions.CreateCallback('identity:registerIdentity', function(source, cb, data)
