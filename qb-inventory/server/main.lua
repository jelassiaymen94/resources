	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					...
					TriggerEvent("qb-log:server:CreateLog", "adminitem", "QB INVENTORY", "black", "**" .. source .. "** GAVE " .. id .. " " .. itemData["name"] .. " " .. amount .. "")
				else
...
QBCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, _)
	...
	for _ = 1, 10, 1 do
		...
		if AddItem(source, randitem["name"], amount) then
            ...
            Wait(500)
			TriggerEvent("qb-log:server:CreateLog", "adminitem", "QB INVENTORY", "black", "**" .. source .. "** GAVE " .. randitem["name"] .. " " .. amount .. "")
		end
	end
end, "god")
