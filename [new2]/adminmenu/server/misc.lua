-- Ban Player
RegisterNetEvent('adminmenu:server:BanPlayer', function(data, selectedData)
...
-- Warn Player
RegisterNetEvent('adminmenu:server:WarnPlayer', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
RegisterNetEvent('adminmenu:server:KickPlayer', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    local src = source
...
-- Revive Player
RegisterNetEvent('adminmenu:server:Revive', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    local player = selectedData["Player"].value
...
-- Revive All
RegisterNetEvent('adminmenu:server:ReviveAll', function(data)
    if not CheckPerms(data.perms) then return end
...
-- Revive Radius
RegisterNetEvent('adminmenu:server:ReviveRadius', function(data)
    if not CheckPerms(data.perms) then return end
...
-- Set RoutingBucket
RegisterNetEvent('adminmenu:server:SetBucket', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
-- Give Money
RegisterNetEvent('adminmenu:server:GiveMoney', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
-- Give Money to all
RegisterNetEvent('adminmenu:server:GiveMoneyAll', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
-- Take Money
RegisterNetEvent('adminmenu:server:TakeMoney', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
RegisterNetEvent('adminmenu:server:ToggleBlackout', function(data)
    if not CheckPerms(data.perms) then return end
...
-- Toggle Cuffs
RegisterNetEvent('adminmenu:server:CuffPlayer', function(data, selectedData)
    if not CheckPerms(data.perms) then return end

    local target = selectedData["Player"].value

    TriggerClientEvent('adminmenu:client:ToggleCuffs', target)
    QBCore.Functions.Notify(source, locale("toggled_cuffs"), 'success')
end)

-- Give Clothing Menu
RegisterNetEvent('adminmenu:server:ClothingMenu', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
