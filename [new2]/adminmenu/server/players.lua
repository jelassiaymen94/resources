...
lib.callback.register('adminmenu:callback:GetPlayers', function(source)
    return getPlayers()
end)

-- Set Job
RegisterNetEvent('adminmenu:server:SetJob', function(data, selectedData)
...
-- Set Gang
RegisterNetEvent('adminmenu:server:SetGang', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    local src = source
...
-- Set Perms
RegisterNetEvent("adminmenu:server:SetPerms", function (data, selectedData)
    if not CheckPerms(data.perms) then return end
    local src = source
