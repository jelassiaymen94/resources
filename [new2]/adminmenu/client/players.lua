...
local function preparePlayers()
    currentPlayers = {}
    Wait(100)
    currentPlayers = lib.callback.await('adminmenu:callback:GetPlayers')
end

-- Toggle Blips and Names events
RegisterNetEvent('adminmenu:client:toggleBlips', function(data)
    if not CheckPerms(data.perms) then return end
    if not ShowBlips then preparePlayers() end
    ToggleBlipsAndNames(true)
end)

RegisterNetEvent('adminmenu:client:toggleNames', function(data)
    if not CheckPerms(data.perms) then return end
...
-- Mute Player
RegisterNetEvent("adminmenu:client:MutePlayer", function (data, selectedData)
    if not CheckPerms(data.perms) then return end
