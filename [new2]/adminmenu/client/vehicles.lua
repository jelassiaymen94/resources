...
-- Own Vehicle
RegisterNetEvent('adminmenu:client:Admincar', function(data)
    ...
    if sharedVehicles then
        TriggerServerEvent('adminmenu:server:SaveCar', props, sharedVehicles, hash, props.plate)
    else
...
-- Spawn Vehicle
RegisterNetEvent('adminmenu:client:SpawnVehicle', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
...
-- Refuel Vehicle
RegisterNetEvent('adminmenu:client:RefuelVehicle', function(data)
    if not CheckPerms(data.perms) then return end
...
-- Change plate
RegisterNetEvent('adminmenu:client:ChangePlate', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    ...
    if cache.vehicle then
        local AlreadyPlate = lib.callback.await("adminmenu:callback:CheckAlreadyPlate", false, plate)
        ...
        TriggerServerEvent('adminmenu:server:ChangePlate', plate, currentPlate)
        Wait(100)
...
RegisterNetEvent('adminmenu:client:ToggleVehDevMenu', function(data)
    if not CheckPerms(data.perms) then return end
    if not cache.vehicle then return end
...
RegisterNetEvent('adminmenu:client:maxmodVehicle', function(data)
    if not CheckPerms(data.perms) then return end
...
RegisterNetEvent("adminmenu:client:SpawnPersonalvehicle", function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    ...
    lib.callback('adminmenu:server:GetVehicleByPlate', false, function(vehModel)
        vehicle = vehModel
    end, plate)
    Wait(100)
