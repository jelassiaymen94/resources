...
CheckOrders = function()
    MySQL.Async.fetchAll('SELECT * FROM dealerships_orders WHERE deliverydate <= date(NOW())', {}, function(result)
        ...
        if #result > 0 and Shared.Debug then
            print("^3[qb-dealerships] ^5"..#result.." vehicles added to stock^7")
        end
    end)
    SetTimeout(60*60*1000, CheckOrders) -- 60 min interval
end

RegisterServerEvent('qb-dealerships:server:withdrawBank', function(amount, society)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.job.name ~= society then return end
    if Player.PlayerData.job.grade.level < Shared.Dealerships[society].moneyMenuThreshold then return end
    ...
    if job == society then
        ...
        if amount <= bank then
            ...
            if Shared.Debug then
                print("^3[qb-dealerships] ^5"..Player.PlayerData.name.." withdrew "..amount..". New balance: "..newBalance.." ("..society..")".."^7")
            end
        else
...
RegisterServerEvent('qb-dealerships:server:depositBank', function(amount, society)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.job.name ~= society then return end
    ...
    if job == society then
        ...
        if amount <= sourceBank then
            ...
            if Shared.Debug then
                print("^3[qb-dealerships] ^5"..Player.PlayerData.name.." deposit "..amount..". New balance: "..newBalance.." ("..society..")".."^7")
            end
        else
...
RegisterNetEvent('qb-dealerships:server:SetJob', function(citizenid, job, type)
    local src = source
    ...
    if sourcePlayer.PlayerData.job.grade.level < Shared.Dealerships[job].bossMenuThreshold then return end
...
RegisterNetEvent('qb-dealerships:server:setOwner', function(vehmodel, hash, target, dealership)
    local src = source
    ...
    if not Shared.Dealerships[seller.PlayerData.job.name] then return end
    ...
    if stock > 0 then
        ...
        TriggerClientEvent('qb-dealerships:client:setOwner', src, newplate)
        ...
        -- LOG
        if Shared.Debug then
            print("^3[qb-dealerships] ^5"..seller.PlayerData.name.." sold "..QBCore.Shared.Vehicles[vehmodel]["brand"].." "..QBCore.Shared.Vehicles[vehmodel]["name"].." to "..buyer.PlayerData.name.." ("..dealership..")".."^7")
        end
    else
...
QBCore.Functions.CreateCallback('qb-dealerships:server:getStockPrice', function(source, cb, dealership)
    local stock = MySQL.query.await('SELECT * FROM dealerships_stock WHERE dealership = ?', {dealership})
    cb(stock)
end)

QBCore.Functions.CreateCallback('qb-dealerships:server:GetDealershipData', function(source, cb, dealership)
    -- Bank
...
QBCore.Functions.CreateCallback('qb-dealerships:server:BuyStock', function(source, cb, dealership, model)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Commands.Add("sellcar", "Sell a car from the showroom (Dealerships only)", {{name = "ID", help = "Player ID"}} , true, function(source, args)
    ...
    if Shared.Dealerships[sourcePlayer.PlayerData.job.name] then
        local target = targetPlayer.PlayerData.source
        local targetName = targetPlayer.PlayerData.name
        local job = sourcePlayer.PlayerData.job.name
        TriggerClientEvent('qb-dealerships:client:sellConfirm', src, target, targetName, job)
    end
end)
...
QBCore.Commands.Add("Testdrive", "Testdrive a car (Dealerships only)", {} , false, function(source, args)
    ...
    if Shared.Dealerships[Player.PlayerData.job.name] then
        TriggerClientEvent("qb-dealerships:client:testdrive", src)
    else
...
RegisterCommand('dealer_debug', function(source)
    ...
    print("^3[qb-dealerships] ^5".."Debug mode: "..tostring(Shared.Debug).."^7")
end, true)

CreateThread(function() 
    -- Check Shared for vehicles
    for dealership, v in pairs(Shared.Dealerships) do
        for brand, vehicles in pairs(v.vehicles) do
            for i=1, #vehicles do
                if not QBCore.Shared.Vehicles[vehicles[i]] then
                    print("^3[qb-dealerships] ^5"..vehicles[i].."("..dealership..") is not in Shared Vehicles!^7")
                end
            end
        end
    end
    ...
    if Shared.Debug then
        print("^3[qb-dealerships] ^5Done checking vehicles in config.^7")
    end
