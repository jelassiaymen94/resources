...
QBCore.Functions.CreateCallback("moneywash:server:GetWashableItems", function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback("moneywash:server:GetDryableItems", function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback("moneywash:server:GetAvailableMoney", function(source, cb, machine, mtype)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('moneywash:server:WashMoney', function(args)
    ...
    if Config.Washable_Items[args.item.name].worth_type == "metadata" then
		...
		Player.Functions.RemoveItem(args.item.name, args.item.amount, args.item.slot)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[args.item.name], "remove", args.item.amount)
	end

    TriggerClientEvent('QBCore:Notify', source, 'You started washing $'..args.item_worth, 'success')

    TriggerEvent('moneywash:server:sendWebhook', 1, 4360181, charactername.." started washing $"..args.item_worth, "Player Name: "..charactername.."\nCitizen ID: "..Player.PlayerData.citizenid.."\nMoney Amount: $"..args.item_worth.."\nWash Time: "..args.washtime.." seconds", "Made by Rebel Scripts")
end)

RegisterNetEvent('moneywash:server:DryMoney', function(args)
    local Player = QBCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    local charactername = Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname
    ...
    TriggerEvent('moneywash:server:sendWebhook', 2, 4360181, charactername.." started drying $"..args.item_worth, "Player Name: "..charactername.."\nCitizen ID: "..Player.PlayerData.citizenid.."\nMoney Amount: $"..args.item_worth.."\nWash Time: "..args.drytime.." seconds", "Made by Rebel Scripts")
end)

RegisterNetEvent('moneywash:server:GrabWetMoney', function(args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local charactername = Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname
    ...
    if success then
        ...
        TriggerEvent('moneywash:server:sendWebhook', 1, 4360181, charactername.." grabbed $"..args.wetmoney.." worth of wet money", "Player Name: "..charactername.."\nCitizen ID: "..Player.PlayerData.citizenid.."\nMoney Amount: $"..args.wetmoney, "Made by Rebel Scripts")
    end
end)

RegisterNetEvent('moneywash:server:GrabDryMoney', function(args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local charactername = Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname
    ...
    if success then
        ...
        TriggerEvent('moneywash:server:sendWebhook', 2, 4360181, charactername.." grabbed $"..args.drymoney.." worth of dry money", "Player Name: "..charactername.."\nCitizen ID: "..Player.PlayerData.citizenid.."\nMoney Amount: $"..args.drymoney.."\nGained Experience Points: "..level.experience_knowledge, "Made by Rebel Scripts")
    end
end)