	...
	for k, v in pairs(Config.Locations) do
		if k == "vendingmachine" and Config.VendOverride then
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { { event = "jim-shops:ShopMenu", icon = (v["targetIcon"] or "fa-solid fa-bolt"), label = (v["targetLabel"] or "Browse Shop"), item = v["requiredItem"], job = v["job"] or nil, gang = v["gang"] or nil,
				...
				Targets["Shop - ['"..k.."("..l..")']"] =
					exports['qb-target']:AddCircleZone("Shop - ['"..k.."("..l..")']", vector3(b.x, b.y, b.z), 1.0, { name="Shop - ['"..k.."("..l..")']", debugPoly=Config.Debug, useZ=true, },
...
RegisterNetEvent('jim-shops:ShopMenu', function(data, custom)
	...
	for i = 1, #products do
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
			...
			if Config.JimMenu then setheader = "<img src=nui://"..Config.img..QBCore.Shared.Items[products[i].name].image.." width=30px onerror='this.onerror=null; this.remove();'>"..QBCore.Shared.Items[products[i].name].label
