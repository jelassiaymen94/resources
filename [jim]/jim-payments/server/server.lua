...
QBCore.Commands.Add("polcharge", Loc[Config.Lan].command["charge"], {}, false, function(source) TriggerClientEvent("jim-payments:client:PolCharge", source) 
	TriggerEvent("qb-log:server:CreateLog", "commands", "JIM PAYMENTS", "black", "**" .. source .. "** Used Pol Charge Command")
end)
...
RegisterServerEvent("jim-payments:server:PolPopup", function(data)
	...
	if data.accept == true then
		...
		if Config.ApGov then exports['ap-government']:chargeCityTax(billed.PlayerData.source, "Item", data.amount) end
		TriggerEvent("qb-log:server:CreateLog", "register", "JIM PAYMENTS", "green", "**" .. src .. "** Removed " .. data.amount .. " For a Register Charge")
		...
		TriggerEvent("qb-log:server:CreateLog", "register", "JIM PAYMENTS", "black", "**" .. src .. "** Got " .. commission .. " from a Register Charge")
