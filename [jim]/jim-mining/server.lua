...
RegisterServerEvent('Polar-Mining:BreakBit', function()
    ...
    Player.Functions.RemoveItem('drillbit', randomChance, false, {["quality"] = nil})
    TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items["drillbit"], "remove", randomChance)
...
RegisterServerEvent('jim-mining:MineReward', function()
	...
	TriggerEvent("qb-log:server:CreateLog", "jobitemadded", "JIM MINING", "red", "**" .. Player .. "** Add " .. randomChance .. " charcoal")
...
RegisterServerEvent('jim-mining:CrackReward', function(cost)
	...
	for i = 1, math.random(1,3) do
		...
		if Player.Functions.AddItem(randItem, amount) then
			...
			TriggerEvent("qb-log:server:CreateLog", "jobitemadded", "JIM MINING (Crack)", "yellow", "**" .. Player .. "** Add " .. amount .. " " .. randItem .. "")
...
RegisterServerEvent('jim-mining:WashReward', function(cost)
	...
	for i = 1, math.random(1,3) do
		...
		TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[randItem], 'add', amount)
		TriggerEvent("qb-log:server:CreateLog", "jobitemadded", "JIM MINING (Wash)", "green", "** Add " .. amount .. " " .. randItem .. "")
		randItem = nil
...
RegisterServerEvent('jim-mining:PanReward', function()
	...
	for i = 1, math.random(1,3) do
		...
		Player.Functions.AddItem(randItem, amount)
		TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[randItem], 'add', amount)
		TriggerEvent("qb-log:server:CreateLog", "jobitemadded", "JIM MINING (Pan)", "yellow", "**" .. Player .. "** Add " .. amount .. " " .. randItem .. "")
		randItem = nil
	end
end)
...
RegisterNetEvent("jim-mining:Selling", function(data)
    ...
    if Player.Functions.GetItemByName(data.item) ~= nil then
		...
		TriggerEvent("qb-log:server:CreateLog", "jobmoneygiven", "JIM MINING", "black", "**" .. Player .. "** Given " .. pay .. " cash")
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[data.item], 'remove', amount)
		TriggerEvent("qb-log:server:CreateLog", "jobitemremoved", "JIM MINING", "red", "**" .. Player .. "** Removed" .. amount .. "" .. data.item .. "")
    else
