...
local math2 = math.random(2500,4000)
...
RegisterNetEvent('hospital:client:UseIfaks', function()
    ...
    QBCore.Functions.Progressbar("use_bandage", "Using Ifak", 2500, false, true, {
...
end)]]
{
    ...
    Wait(10000)
    SetEntityHealth(ped, GetEntityHealth(ped) + 6)
    Wait(10000)
    SetEntityHealth(ped, GetEntityHealth(ped) + 7)
    Wait(10000)
...
RegisterNetEvent('hospital:client:UseBandage', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar("use_bandage", 'Using First Aid', 3500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        SetEntityHealth(ped, GetEntityHealth(ped) + 19)
        if math.random(1, 100) < 50 then
            RemoveBleed(1)
        end
        if math.random(1, 100) < 7 then
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar("use_bandage", Lang:t('progress.painkillers'), 2500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
