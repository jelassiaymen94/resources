...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://Polar-Clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://Polar-Clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://Polar-Clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://Polar-Clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        $.post('https://Polar-Clothing/setupCam', JSON.stringify({
            value: camValue
        }));
    } else {
        if (selectedCam == this) {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://Polar-Clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://Polar-Clothing/rotateLeft');
            break;
        case 87: // W
            $.post('https://Polar-Clothing/CamUp');
            break;
        case 83: // S
            $.post('https://Polar-Clothing/CamDown');
            break;            
    }
});
...
$(document).on('click', "#save-menu", function(e){
    ...
    QBClothing.Close();
    $.post('https://Polar-Clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('https://Polar-Clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://Polar-Clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://Polar-Clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
QBClothing.Close = function() {
    $.post('https://Polar-Clothing/close');
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://Polar-Clothing/saveOutfit', JSON.stringify({
        ...
        $(document).mousemove(function(e) {
            ...
            if (e.pageX < mX) {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://Polar-Clothing/rotateLeftChar');
                    }
                 // moving character right
            } else {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://Polar-Clothing/rotateRightChar');
                    }
            }
            mX = e.pageX
        });
...
$(document).ready(function() {
    $('#rotate-char').bind('mousewheel', function(e){
        if(e.originalEvent.wheelDelta /120 > 0) {
            ...
            $.post('https://Polar-Clothing/fovup');
        }
        else{
            // console.log('scrolling down !');
            $.post('https://Polar-Clothing/fovdown');
        }
    });
});
