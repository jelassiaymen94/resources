...
RegisterNetEvent('Polar-Clothing:client:getOutfits', function(requiredJob, gradeLevel)
    local gender = "male"
    if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    QBCore.Functions.TriggerCallback('Polar-Clothing:server:getOutfits', function(result)
        openMenu({
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            if v.isGang then
                action = function()
                    ...
                    TriggerEvent('Polar-Clothing:client:getOutfits', v.requiredJob, gradeLevel)
                end
            else
                action = function()
                    customCamLocation = v.cameraLocation
            ...
            if inZone then
                ...
                elseif zoneName == 'barber' then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('Polar-Clothing:client:getOutfits', clothingRoom.requiredJob, gradeLevel)
                    end
                end
            end
            Wait(sleep)
...
RegisterNetEvent('Polar-Clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('Polar-Clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('Polar-Clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNetEvent('Polar-Clothing:client:openMenu')
AddEventHandler('Polar-Clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('Polar-Clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)

RegisterNetEvent('Polar-Clothing:client:reloadOutfits')
AddEventHandler('Polar-Clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('Polar-Clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('Polar-Clothing:server:removeOutfit', data.outfitName, data.outfitId)
    ...
    Citizen.CreateThread(function()
        ...
        for k, v in pairs(skinData) do
            ...
            elseif skin == "mp_f_freemode_01" then
                ChangeVariation({
                    clothingType = k,
                    articleNumber = v.fdefaultTexture,
                    type = "texture",
                })
            else
                if k ~= "face" and k ~= "hair" and k ~= "face2" then
...
function SaveSkin()
    ...
    TriggerServerEvent("Polar-Clothing:saveSkin", model, clothing)
end

RegisterNetEvent('Polar-Clothes:client:CreateFirstCharacter')
AddEventHandler('Polar-Clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("Polar-Clothes:loadSkin")
AddEventHandler("Polar-Clothes:loadSkin", function(_, model, data)
...
RegisterNetEvent('Polar-Clothing:client:loadPlayerClothing')
AddEventHandler('Polar-Clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('Polar-Clothing:client:loadOutfit')
AddEventHandler('Polar-Clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("Polar-Clothing:client:adjustfacewear")
AddEventHandler("Polar-Clothing:client:adjustfacewear",function(type)
...
RegisterNetEvent('Polar-Clothing:client:Refreshskin', function()
...
function reloadSkin(health)
    ...
    TriggerServerEvent("Polar-Clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("Polar-Clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
