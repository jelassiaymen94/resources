...
RegisterNetEvent("spikestrips:setPolice")
AddEventHandler("spikestrips:setPolice", function()
    isPolice = true
end)
...
CreateThread(function()
    ...
    if Config.RequireJobPlace then
        if Config.Framework == "esx" or Config.Framework == "qb" then
            ...
            TriggerServerEvent("spikestrips:checkPolice")
        end
    else
        isPolice = true
    end
    ...
    CreateThread(function()
        while true do
            ...
            if (isPolice or not Config.RequireJobRemove) and IsPedOnFoot(PlayerPedId()) then
                if (isPolice or not Config.RequireJobRemove) and DoesEntityExist(closestStinger) and closestStingerDistance <= 4.0 and IsPedOnFoot(PlayerPedId()) then
                    Wait(0)
                    exports['qb-core']:DrawText('[E] Remove', 'left')
