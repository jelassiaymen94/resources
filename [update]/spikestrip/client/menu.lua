if Config.Menu.Enabled then
    CreateThread(function()
        ...
        if Config.Menu.Command then
            RegisterCommand(Config.Menu.Command, function()
                if isPolice or not Config.RequireJobRemove then
                    TriggerEvent("spikestrips:spikestripMenu")
                else
        ...
        if Config.Framework == "esx" and not Config.FrameworkFeatures.UseWarmenu then
            ...
            RegisterNetEvent("spikestrips:spikestripMenu")
            AddEventHandler("spikestrips:spikestripMenu", function()
                if isPolice or not Config.RequireJobRemove then
                    ...
                    local elements = {
                        {
                            ...
                            params = {
                                event = "spikestrips:placeSpikestrip",
                            }
                        }
                    }
                    if isPolice or not Config.RequireJobRemove then
                        table.insert(elements, {
                            ...
                            params = {
                                event = "spikestrips:removeSpikestrip",
                            }
                        })
                    end
                    table.insert(elements, {
