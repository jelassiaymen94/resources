...
RegisterNetEvent("spikestrips:checkPolice")
AddEventHandler("spikestrips:checkPolice", function()
    local src = source

    for k, v in pairs(GetPlayerIdentifiers(src)) do
        if IsPolice(v) then
            TriggerClientEvent("spikestrips:setPolice", src)
            break
        end
    end
end)
    ...
    if Config.FrameworkFeatures.Item then
        if Config.Framework == "esx" then
            ESX.RegisterUsableItem(Config.FrameworkFeatures.Item, function(src)
                ...
                TriggerClientEvent("spikestrips:placeSpikestrip", src)
            end)
        elseif Config.Framework == "qb" then
        ...
        if Config.FrameworkFeatures.ReceiveRemove then
            ...
            RegisterNetEvent("spikestrips:removedSpike")
            AddEventHandler("spikestrips:removedSpike", function()
