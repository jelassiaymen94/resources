...
RegisterNUICallback('wheelsetting', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 then
		...
		TriggerEvent('QBCore:Notify',"Vehicle Stance Saved", "success")
	end
	cb(true)
end)
...
RegisterCommand('stance', function ()
	...
	if Player.job.name == 'bennys' then
...
function OpenStancer()
	...
	if busy or not ent.stancer then TriggerEvent('QBCore:Notify',"No Stancer Kit Installed (F8 FixStance)","error") return end
	local Player = QBCore.Functions.GetPlayerData()
	SetNuiFocus(false, false)
	...
	if vehicle  ~= 0 and #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(vehicle )) < 15 and GetVehicleDoorLockStatus(vehicle ) == 1 then
		...
		if GetVehicleDoorLockStatus(vehicle ) ~= 1 then
			TriggerEvent('QBCore:Notify',"No Unlocked Vehicle Nearby")
		else
			TriggerEvent('QBCore:Notify',"No Nearby Vehicle")
		end
	end
end
