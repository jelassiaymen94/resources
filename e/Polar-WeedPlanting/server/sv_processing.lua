...
RegisterNetEvent('Polar-WeedPlanting:server:ProcessBranch', function()
    local src = source
...
RegisterNetEvent('Polar-WeedPlanting:server:PackageWeed', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateUseableItem(Shared.BranchItem, function(source)
    TriggerClientEvent('Polar-WeedPlanting:client:UseBranch', source)
end)

QBCore.Functions.CreateUseableItem(Shared.WeedItem, function(source, item)
    if item and item.amount >= 20 then
        TriggerClientEvent('Polar-WeedPlanting:client:UseDryWeed', source)
    else
