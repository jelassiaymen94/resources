...
RegisterNetEvent('Polar-WeedPlanting:server:CollectPackageGoods', function()
    local src = source
    ...
    elseif packageCache[citizenid] == 'done' then
        ...
        TriggerClientEvent('Polar-WeedPlanting:client:PackageGoodsReceived', src)
...
RegisterNetEvent('Polar-WeedPlanting:server:DestroyWaitForPackage', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local citizenid = Player.PlayerData.citizenid
...
RegisterNetEvent('Polar-WeedPlanting:server:WeedrunDelivery', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('Polar-WeedPlanting:server:PackageGoods', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
