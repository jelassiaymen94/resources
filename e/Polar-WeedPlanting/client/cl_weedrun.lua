...
local createDropOffPed = function(coords)
	...
	exports['qb-target']:AddTargetEntity(deliveryPed, {
		options = {
			{
				type = 'client',
				event = 'Polar-WeedPlanting:client:DeliverWeed',
...
RegisterNetEvent('Polar-WeedPlanting:client:StartPackage', function(data)
    ...
    QBCore.Functions.Progressbar('weedrun_pack', _U('handing_over_weed'), 4000, false, true, {
        ...
        QBCore.Functions.TriggerCallback('Polar-WeedPlanting:server:PackageGoods', function(result)
            ...
            packageZone:onPlayerInOut(function(isPointInside, point)
                if not isPointInside then
                    ...
                    if waitingForPackage then
                        TriggerServerEvent('Polar-WeedPlanting:server:DestroyWaitForPackage')
...
RegisterNetEvent('Polar-WeedPlanting:client:PackageGoodsReceived', function()
    if waitingForPackage then
        packageZone:destroy()
        waitingForPackage = false
    end
...
RegisterNetEvent('Polar-WeedPlanting:client:ClockIn', function()
...
RegisterNetEvent('Polar-WeedPlanting:client:ClockOut', function()
...
RegisterNetEvent('Polar-WeedPlanting:client:DeliverWeed', function()
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(deliveryPed)) < 5.0 then
		...
		Wait(1000)
		TriggerServerEvent('Polar-WeedPlanting:server:WeedrunDelivery')
    ...
    exports['qb-target']:SpawnPed({
        ...
        target = {
            options = {
                { -- Create Package
                    type = 'client',
                    event = 'Polar-WeedPlanting:client:StartPackage',
                ...
                { -- Receive Package
                    type = 'server',
                    event = 'Polar-WeedPlanting:server:CollectPackageGoods',
                    icon = 'fas fa-circle-chevron-right',
                ...
                { -- Clock In for deliveries
                    type = 'client',
                    event = 'Polar-WeedPlanting:client:ClockIn',
                ...
                { -- Clock out for deliveries
                    type = 'client',
                    event = 'Polar-WeedPlanting:client:ClockOut',
                    icon = 'fas fa-stopwatch',
