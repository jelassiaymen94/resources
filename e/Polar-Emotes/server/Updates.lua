if Config.CheckForUpdates then
    local function VersionLog(_type, log)
        ...
        print(('^8[Polar-Emotes]%s %s^7'):format(color, log))
    end

    local function CheckMenuVersion()
        PerformHttpRequest('https://raw.githubusercontent.com/TayMcKenzieNZ/Polar-Emotes/master/version.txt', function(err, text, headers)
