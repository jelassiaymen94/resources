...
--#region ptfx
RegisterNetEvent("Polar-Emotes:ptfx:sync", function(asset, name, offset, rot, bone, scale, color)
    if type(asset) ~= "string" or type(name) ~= "string" or type(offset) ~= "vector3" or type(rot) ~= "vector3" then
        print("[Polar-Emotes] ptfx:sync: invalid arguments for source:", source)
        return
    end
...
RegisterNetEvent("Polar-Emotes:ptfx:syncProp", function(propNet)
    local srcPlayerState = Player(source).state
    if propNet then
