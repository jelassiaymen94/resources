...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Open Polar-Emotes menu (F4) by default. This may differ from server to server.')
...
if Config.MenuKeybindEnabled then
    ...
    RegisterKeyMapping("emoteui", "Open Polar-Emotes menu", "keyboard", Config.MenuKeybind)
else
...
function OnEmotePlay(EmoteName, textureVariation)
    ...
    elseif animOption then
        if animOption.EmoteLoop then
            MovementType = 0 --(ToggleEmoteMovement and 51 or 1)
        elseif animOption.EmoteStuck then
            MovementType = 0 --(ToggleEmoteMovement and 50 or 1) -- 110010
        else
            MovementType = 0 --(ToggleEmoteMovement and 51 or 1)
        end
    else
        MovementType = 0 -- (ToggleEmoteMovement and 51 or 0)
    end
    ...
    if animOption then
        ...
        if animOption.PtfxAsset then
            ...
            TriggerServerEvent("Polar-Emotes:ptfx:sync", PtfxAsset, PtfxName, vector3(Ptfx1, Ptfx2, Ptfx3),
    ...
    -- AnimationDuration
    TaskPlayAnim(PlayerPedId(), ChosenDict, ChosenAnimation, 5.0, 5.0, -1, MovementType, 0, false, false, false)
    ...
    if animOption and animOption.Prop then
            ...
            if animOption.PtfxAsset and not PtfxNoProp then
                TriggerServerEvent("Polar-Emotes:ptfx:syncProp", ObjToNet(prop))
        end
    end
end
