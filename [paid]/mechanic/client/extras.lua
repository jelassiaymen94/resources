...
RegisterNetEvent("jim-mechanic:client:CarWax", function(time)
	...
	Wait(1000)
	emptyHands(playerPed, true)
	if cleaning == false and time ~= 0 then
		TriggerServerEvent("jim-mechanic:server:WaxActivator", VehToNet(vehicle), time)
	end
	if Config.CosmeticRemoval then toggleItem(false, "cleaningkit") end
end)
...
RegisterNetEvent('jim-mechanic:client:cleanVehicle', function(skip)
	...
	if DoesEntityExist(vehicle) then
		local CleanMenu = {
			...
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head1, params = { event = "jim-mechanic:client:CarWax", args = 0 }}
		if Config.WaxFeatures then
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head2, params = { event = "jim-mechanic:client:CarWax", args = 1800 }}
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head3, params = { event = "jim-mechanic:client:CarWax", args = 3600 }}
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head4, params = { event = "jim-mechanic:client:CarWax", args = 5400 }}
		end
				...
				if GetPedInVehicleSeat(veh, -1) == ped then
					...
					if veh ~= prevVeh then
						...
						if VehicleNitrous[trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())))] then
							TriggerServerEvent('jim-mechanic:database:UpdateNitroLevel', trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId()))), VehicleNitrous[trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())))].level) -- Update the nos when you are no longer in that vehicle
							nos = {
						...
						if not odocalc then odocalc = true
							while odocalc do
								if ShowOdo and IsPedInAnyVehicle(PlayerPedId(), true) then
									...
									local lights = ""
										if dist ~= "" then
											if Config.distkph then odotext = string.format("%05d", math.floor((dist+math.round(DistAdd)) * 0.01)).." <b>Km</b>"
											else odotext = string.format("%05d", math.floor((dist+math.round(DistAdd)) * 0.00621371)).." <b>Mi</b>" end
										end
