...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			Targets[#Targets] =
			exports['qb-target']:AddBoxZone("CatGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="CatGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=out.z-1.03+1.3 },
				{ options = { { event = "jim-catcafe:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].target["job_veh"], job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
				distance = 2.0 })
...
RegisterNetEvent('jim-catcafe:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-catcafe:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan].menu["out_veh"],
										...
										params = { event = "jim-catcafe:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan].menu["remove_veh"], params = { event = "jim-catcafe:client:RemSpawn" } }
	else
		...
		for k,v in pairs(data.list) do
			...
			vehicleMenu[#vehicleMenu+1] = { header = v, params = { event = "jim-catcafe:client:SpawnList", args = { spawnName = spawnName, coords = data.coords } } }
		end
	end
    exports['qb-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent("jim-catcafe:client:SpawnList", function(data)
...
RegisterNetEvent("jim-catcafe:client:RemSpawn", function(data)
...
RegisterNetEvent("jim-catcafe:client:Garage:Blip", function(data)
...
AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end
	for k, v in pairs(Config.Locations) do if v.garage then exports['qb-target']:RemoveZone("CatGarage: "..k) end end
