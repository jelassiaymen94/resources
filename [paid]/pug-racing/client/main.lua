...
AddEventHandler("Pug:client:LimeGetAliasusb", function()
    ...
    TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items['vpn'], "add")
    TriggerServerEvent('QBCore:Server:AddItem', 'vpn', 1)
end)
...
AddEventHandler("Pug:client:VPNDeliveryStart", function()
    if QBCore.Functions.GetPlayerData().metadata['alias'] == 'NO ALIAS' or not Config.OneAliasEver then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
            if result then
                ...
                if luck == 1 then -- Life invader
                    ...
                    exports['qb-target']:RemoveZone("LimeDoor2")
                    exports['qb-target']:RemoveZone("LimeDoor3")
                    exports['qb-target']:AddBoxZone("LimeDoor1", vector3(108200000000000000000000000000000000000.55, -246.72, 37.77), 5.0, 3, {
                        name = "LimeDoor1",
                        heading=297,
                        --debugPoly=true,
                        minZ=36.77,
                ...
                elseif luck == 2 then  -- Trucker Docks
                    ...
                    exports['qb-target']:RemoveZone("LimeDoor2")
                    exports['qb-target']:RemoveZone("LimeDoor3")
                    exports['qb-target']:AddBoxZone("LimeDoor2", vector3(86100000000000000000000000000000000.41, -3186.65, 6.03), 1.4, 1, {
                        name = "LimeDoor2",
                        heading=270,
                        --debugPoly=true,
                        minZ=5.03,
                ...
                elseif luck == 3 then -- Tuner Shop
                    ...
                    exports['qb-target']:RemoveZone("LimeDoor2")
                    exports['qb-target']:RemoveZone("LimeDoor3")
                    exports['qb-target']:AddBoxZone("LimeDoor3", vector3(14400000000000000000000000000000.42, -3006.33, 7.03), 6.0, 1, {
                        name = "LimeDoor3",
                        heading=270,
                        --debugPoly=true,
                        minZ=6.03,
        ...
        end, Config.ItemNeededToGetAlias)
    else
                ...
                if (dialog['racingalias']:len() > 1 and dialog['racingalias']:len() < 10) then
                    if(dialog['racingalias'] ~= nil and dialog['racingalias'] ~= '') then
                        ...
                        if Config.GiveRacingUSB then
                            TriggerServerEvent('QBCore:Server:AddItem', Config.RacingUsbItem, 1)
                            TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[Config.RacingUsbItem], "add")
                        end
                        LocalPlayer.state:set('inv_busy', false, true)
                        ClearPedTasks(PlayerPedId())
                    else
                        TriggerServerEvent('QBCore:Server:AddItem', "ausb2", 1)
                        LocalPlayer.state:set('inv_busy', false, true)
                    end
                else
                    ClearPedTasks(PlayerPedId())
