...
RegisterNetEvent('jim-mechanic:client:applyBrakes', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 12) == level then triggerNotify(nil, QBCore.Shared.Items["brakes"..level].label.." "..Loc[Config.Lan]["common"].already, "error") 
		elseif GetVehicleMod(vehicle, 12) < (level-1) then triggerNotify(nil, "You need to install a Lower Level First", "error") 
		elseif GetVehicleMod(vehicle, 12) > level then triggerNotify(nil, "You cant install a Lower Engine", "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
...
RegisterNetEvent('jim-mechanic:client:applySuspension', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if currentSuspension == level then triggerNotify(nil, QBCore.Shared.Items["suspension"..level+1].label.." "..Loc[Config.Lan]["common"].already, "error") 
		elseif currentSuspension < (level-1) then triggerNotify(nil, "You need to install a Lower Level First", "error") 
		elseif currentSuspension > level then triggerNotify(nil, "You cant install a Lower Engine", "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
...
RegisterNetEvent('jim-mechanic:client:applyTransmission', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		for _, v in pairs({"engine"}) do
			if #(GetEntityCoords(PlayerPedId()) - GetWorldPositionOfEntityBone(vehicle, GetEntityBoneIndexByName(vehicle, v))) <= 2.0 then
				lookVeh(GetWorldPositionOfEntityBone(vehicle, GetEntityBoneIndexByName(vehicle, v)))
				found = true
				break
			end
		...
		if currentTrans == level then triggerNotify(nil, QBCore.Shared.Items["transmission"..level+1].label.." "..Loc[Config.Lan]["common"].already, "error") 
		elseif currentTrans < (level-1) then triggerNotify(nil, "You need to install a Lower Level First", "error") 
		elseif currentTrans > level then triggerNotify(nil, "You cant install a Lower Engine", "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			SetVehicleDoorOpen(vehicle, 4, false, false)
			QBCore.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing.."LVL: "..level+1, math.random(7000,10000), false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = true, },
			{ animDict = "mini@repair",	anim = "fixing_a_ped", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
...
RegisterNetEvent('jim-mechanic:client:applyEngine', function(level)
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 11) == level then triggerNotify(nil, "LVL: "..(level+1)..Loc[Config.Lan]["common"].already, "error") 
			elseif GetVehicleMod(vehicle, 11) < (level-1) then triggerNotify(nil, "You need to install a Lower Level First", "error") 
			elseif GetVehicleMod(vehicle, 11) > level then triggerNotify(nil, "You cant install a Lower Engine", "error") else
			SetVehicleEngineOn(vehicle, false, false, true)
			SetVehicleDoorOpen(vehicle, 4, false, false)
