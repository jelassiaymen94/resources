...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", Lang:t("progressbar.robbing"), math.random(15000, 22000), false, true, {
...
RegisterNetEvent('police:client:JailPlayer', function()
        ...
        if tonumber(dialog['jailtime']) > 10 then
            if tonumber(dialog['personid']) > 0 then
            TriggerServerEvent("police:server:JailPlayer", tonumber(dialog['personid']), tonumber(dialog['jailtime']))
            else
                QBCore.Functions.Notify("No one has a 0 id", "error")
            end
        else
            QBCore.Functions.Notify("Send them for more time", "error")
        end
    
end)

RegisterNetEvent('Polar-Police:client:CallSign', function()
    ...
    if tonumber(dialog['sign']) > 0 then
        TriggerServerEvent("Polar-Police:Server:SetSign", tonumber(dialog['sign']))
    else
        QBCore.Functions.Notify("needs to be higher than 0", "error")
    end
...
RegisterNetEvent('police:client:PutPlayerInVehicle', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if not isHandcuffed and not isEscorted then
            QBCore.Functions.Progressbar("putincar", "Putting in Vehicle!", 4500, false, true, {
                disableMovement = true,
...
RegisterNetEvent('police:client:SetPlayerOutVehicle', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            QBCore.Functions.Progressbar("takeoutofcar", "Taking out of Vehicle!", 4500, false, true, {
                disableMovement = true,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
...
RegisterNetEvent('police:client:EscortPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            QBCore.Functions.Progressbar("escorting", "Escorting!", 1000, false, true, {
                disableMovement = true,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
