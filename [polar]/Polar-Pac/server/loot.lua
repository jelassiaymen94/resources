...
function givet(item, min, max)
    if item == 'markedbills' then
        ...
        TriggerEvent("qb-log:server:CreateLog", "Pacific", "Pacific Bank Robbery", "red", "**".. Player.PlayerData.name .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*)"..' Got ' .. QBCore.Shared.Items[item] .. ' from robbin ' .. chance .. '')
    else
    local src = source local Player = QBCore.Functions.GetPlayer(src)
...
function give(item, amount)
    if item == 'markedbills' then
        ...
        TriggerEvent("qb-log:server:CreateLog", "Pacific", "Pacific Bank Robbery", "red", "**".. Player.PlayerData.name .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*)"..' Got ' .. QBCore.Shared.Items[item] .. ' from robbin ')
    else
        local src = source local Player = QBCore.Functions.GetPlayer(src)
        Wait(150)
        if amount == nil then
            ...
            TriggerEvent("qb-log:server:CreateLog", "Pacific", "Pacific Bank Robbery", "red", "**".. Player.PlayerData.name .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*)"..' Got ' .. QBCore.Shared.Items[item] .. ' from robbin ' .. amount .. '')
