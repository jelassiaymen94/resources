$(function() {
    ...
    window.addEventListener('message', function(e) {
        ...
        if (item.action == "start") {
            ...
            $('.hackBox').fadeIn(500);
            $('#textInfo').html("Login Codes Required");
    ...
    const gameOver = () => {
        check_passwoord = false;
        $('.hackInfo').css("display", "block");
        $('#textInfo').html("Hack Failed");
        $(".hackFunction").css("display", "none");
        $(".hackText").css("display", "none");
        progressBarStart('end', 5);
        setTimeout(function(){
            $.post('https://memory/fail');
        }, 3000);
    };
        ...
        const process = () => {
            if (width > 0) {
                ...
                if (type == 'start') {
                    ...
                    $('#textInfo1').html("Bank Login Codes");
