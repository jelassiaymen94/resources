...
local toggle_rappel = 101 -- control id to rappel out of the heli. Default: INPUT_DUCK (X)
...
AddEventHandler('heli:spotlight', function(serverID, state)
	if DoesPlayerExist(serverID) then
		...
		Trace("Set heli light state to "..tostring(state).." for serverID: "..serverID)
	end
end)
