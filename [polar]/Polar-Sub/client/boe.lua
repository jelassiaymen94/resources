...
local bluedrawables = '{"1":["masks",-1],"2":["hair",2],"3":["torsos",6],"4":["legs",9],"5":["bags",0],"6":["shoes",1],"7":["neck",0],"8":["undershirts",15],"9":["vest",0],"10":["decals",0],"11":["jackets",486],"0":["face",11]}'
local blueprops = '{"1":["glasses",0],"2":["earrings",-1],"3":["mouth",-1],"4":["lhand",-1],"5":["rhand",-1],"6":["watches",-1],"7":["braclets",-1],"0":["hats",72]}'
...
function penis()    
    ...
    veh = CreateVehicle(car, 1056.84, -387.26, 67.86, 220.59, false, false)
...
CreateThread(function()
    while true do
        ...
        local pedPos = GetEntityCoords(blue)
        local distance = GetDistanceBetweenCoords(playerPos, pedPos, true)

        if distance < 1.5 and IsControlJustPressed(0, 38) then -- 38 is the "E" key on the keyboard
            
            -- vector4(1058.83, -387.24, 67.85, 311.16)
            SetEntityCoords(PlayerPedId(), vector4(1058.83, -387.24, 66.85, 310.16))
            SetEntityHeading(PlayerPedId(), 311.16)
           
            loadAnimDict("mp_ped_interaction")
            
            TaskPlayAnim(PlayerPedId(), "mp_ped_interaction", "kisses_guy_a", 1.0, 1.0, -1, 1, 0, 0, 0, 1)
            TaskPlayAnim(blue, "mp_ped_interaction", "kisses_guy_a", 1.0, 1.0, -1, 1, 0, 0, 0, 1)
            Wait(5000)
            ClearPedTasksImmediately(PlayerPedId())
            ClearPedTasksImmediately(blue)
            --[[
            ClearPedTasksImmediately(blue)
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_AA_COFFEE", 1, true)
            --Wait(2000)
            TaskStartScenarioInPlace(blue, "WORLD_HUMAN_AA_COFFEE", 1, true)
            Wait(6000)
            ClearPedTasksImmediately(PlayerPedId())
            ClearPedTasksImmediately(blue)]]
    ...
    if IsModelInCdimage(model) and IsModelValid(model) then
        ...
        if setDefault and model ~= nil and not isCustomSkin(model) then
            if (model ~= `mp_f_freemode_01` and model ~= `mp_m_freemode_01`) then
                ...
                SetPedComponentVariation(blue, 1, 10, 10, 10)
