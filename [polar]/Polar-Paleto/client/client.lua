...
local laserdrillitem = 'laserdrill' -- laser drill to cutout glass
local yellowdrillitem = 'drill' -- item for deposit boxes (2 sections, yellow drill)
local ydrillitemchance = 25 -- chance for yellow drill to break
local drillitem = 'drill' -- item for deposit boxes (regular)
local drillitemchance = 25 -- chance for item to break
...
function grabloot(door, object)
    ...
    sceneObject = GetClosestObjectOfType(trolylp[door], 2.0, GetHashKey(trollys[door]), 0, 0, 0)
    print(sceneObject)
...
RegisterNetEvent('Polar-Paleto:Client:Target', function(data) 
   ...
   -- print(GetEntityRotation(doors[p]))
    ...
    if tablecheck(p, trollytable) then
        --print(doors[p])
        TriggerServerEvent('Polar-Paleto:Server:TrollyModelSync', p, GetEntityCoords(doors[p]))
        Wait(500)
        grabloot(p, doors[p])
    else
        Animation(p, doors[p])  
