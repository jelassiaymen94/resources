...
RegisterNetEvent('Polar-Paleto:client:start', function()  pp = vector4(-109.43, 6483.32, 31.47, 224.11) door = startdoorname coords = vector3(-109.46, 6484.36, 31.27)
...
function drill(drillpos, drillrot, door) local pedCo = GetEntityCoords(PlayerPedId()) LocalPlayer.state:set('inv_busy', true, true) local coords, pedRotation = GetEntityCoords(PlayerPedId()), GetEntityRotation(PlayerPedId()) local animDict = 'anim_heist@hs3f@ig9_vault_drill@laser_drill@' loadAnimDict(animDict) local bagModel = bagcolor loadModel(bagModel) local laserDrillModel = 'hei_prop_heist_drill' loadModel(laserDrillModel) RequestAmbientAudioBank('DLC_HEIST_FLEECA_SOUNDSET', 0) RequestAmbientAudioBank('DLC_MPHEIST\\HEIST_FLEECA_DRILL', 0) 
    ...
    TriggerServerEvent('Polar-Paleto:Drill', door)
...
function other()
    ...
    CreateTarget(fingerprintdoor, Config.FingerEye, "Polar-Paleto:Client:VaultHack", "Hack", "fas fa-bolt", Config.Debug)
...
function grabloot(door, object)
    ...
    local scene1 = NetworkCreateSynchronisedScene(GetEntityCoords(object2), GetEntityRotation(object2), 2, true, false, 1065353216, 0, 1.3)
    ...
    local scene2 =  NetworkCreateSynchronisedScene(GetEntityCoords(object2), GetEntityRotation(object2), 2, true, false, 1065353216, 0, 1.3)
    ...
    local scene3 =  NetworkCreateSynchronisedScene(GetEntityCoords(object2), GetEntityRotation(object2), 2, true, false, 1065353216, 0, 1.3)
...
RegisterNetEvent('Polar-Paleto:Client:VaultHack', function(data) 
    if playeritem(vaultitem) then
        if data.id == fingerprintdoor then
            callback('Polar-Paleto:FingerCheck', function(result) if result then 
...
function starttarget()
    ...
    if oxt then
        ...
        options = {{  name = startdoorname, icon = "fas fa-fire", label = "Thermite", event = 'Polar-Paleto:client:start', canInteract = function(_, distance) return distance <= Config.OxTargetDistance end }, } })
    else
...
RegisterNetEvent('Polar-Paleto:Client:StartTargets', function()
    ...
    CreateTarget(door1name, vec3(Config.Door1Eye.x, Config.Door1Eye.y, Config.Door1Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)
    CreateTarget(door2name, vec3(Config.Door2Eye.x, Config.Door2Eye.y, Config.Door2Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)
    CreateTarget(door3name, vec3(Config.Door3Eye.x, Config.Door3Eye.y, Config.Door3Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)
...
RegisterNetEvent('Polar-Paleto:Client:ResetDoors', function()
    if oxd then
        doorlock(door1name, 1)
        doorlock(door2name, 1)
        doorlock(door3name, 1)
        doorlock(doorcard1name, 1)
        doorlock(doorcard2name, 1)
...
function snatch(name, item, chances)
    ...
    TriggerServerEvent('Polar-Paleto:Server:RemoveProp', name)
