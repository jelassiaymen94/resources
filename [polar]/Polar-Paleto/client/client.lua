...
local proptable = {
    ...
    'paletoprop30', 'paletoprop31', 'paletoprop32', 'paletoprop33', 'paletoprop34', 'paletoprop35', 'paletoprop36', 'paletoprop37', 'paletoprop38', 'paletoprop39', 'paletoprop40', 'paletoprop41',
    'paletoprop42', 'paletoprop43', 'paletoprop44', 'paletoprop45', 'paletoprop46', 'paletoprop47', 'paletoprop48', 'paletoprop49', 'paletoprop50',
}
...
function PlantThermite(pp, door)
    ...
    TriggerServerEvent('Polar-Paleto:Server:StopInteract', door) TriggerServerEvent('Polar-Paleto:Server:RemoveItem', thermiteitem, 1) RequestAnimDict("anim@heists@ornate_bank@thermal_charge") RequestModel(bagcolor)
...
local paletodrill1loc = nil local paletodrill1name = nil local paletodrill2loc = nil local paletodrill2name = nil
function other()
    CreateTarget(vaultdoorname, vaultanimloc, "Polar-Paleto:Client:VaultHack", "Hack", "fas fa-bolt", Config.Debug)
...
RegisterNetEvent('Polar-Paleto:Client:Vault', function(open)
    if Config.VaultDoorlock then  if oxd then  doorlock(vaultdoorname, 0) else TriggerServerEvent('qb-doorlock:server:updateState', vaultdoorname, false, false, false, true, false, false) end return end
    if not open then
    local object = GetClosestObjectOfType(vaultloc, 20.0, vaultid, false, false, false)
    --print(object)
    local entHeading = closed
...
RegisterNetEvent('Polar-Paleto:Client:AddTarget', function(door, prop, var) 
    if oxt then 
        ...
        options = {{  event = "Polar-Paleto:Client:Target", type = door,  icon = "fas fa-bolt", label = "Grab", canInteract = function(_, distance) return distance <= Config.OxTargetDistance end }, } })
        ...
        { options = {{ event = "Polar-Paleto:Client:Target", type = door,  icon = "fas fa-bolt", label = "Grab"}}, distance = 1.0  }) 
    end
end)
RegisterNetEvent('Polar-Paleto:Client:AddPickupTarget', function(door, prop, var, pile) 
    if oxt then
        targets[door] = exports.ox_target:addBoxZone({ coords = vec3(var.x, var.y, var.z), size = vec3(1, 1, 1), rotation = 1, debug = hi,
        options = {{  event = "Polar-Paleto:Client:PickupTarget", type = door, piles = pile, icon = "fas fa-bolt", label = "Grab", canInteract = function(_, distance) return distance <= Config.OxTargetDistance end }, } })
...
RegisterNetEvent('Polar-Paleto:Client:AddPaintTarget', function(door, prop, var, cases) 
    if oxt then 
        targets[door] = exports.ox_target:addBoxZone({ coords = vec3(var.x, var.y, var.z), size = vec3(1, 1, 1), rotation = 1, debug = hi,
        options = {{  event = "Polar-Paleto:Client:PaintTarget", type = door, case = cases, icon = "fas fa-bolt", label = "Grab", canInteract = function(_, distance) return distance <= Config.OxTargetDistance end }, } })
        if hi then print('paint id: ' ..  targets[door] .. ' propdoor: ' .. door .. ' proplabel: ' .. prop) end

    else
        exports['qb-target']:AddBoxZone(door, vec3(var.x, var.y, var.z), 0.5, 0.5, { name = door, heading = 28.69, debug = hi, minZ = var.z - 1.5, maxZ =  var.z + 1.5,}, 
        { options = {{ event = "Polar-Paleto:Client:PaintTarget", type = door, case = cases, icon = "fas fa-bolt", label = "Grab"}}, distance = 1.0 }) 
    end
end)
...
RegisterNetEvent('Polar-Paleto:Client:HackComputer', function(data)
    if playeritem(computeritem) then
        local door = data.id
        callback('Polar-Paleto:Door' .. door, function(result) if result then 
...
RegisterNetEvent('Polar-Paleto:client:DrillStart', function(drillpos, drillrot, door)
    ...
    if drill(drillpos, drillrot) then
        ...
        local chance = math.random(1,100) if chance <= drillitemchance then TriggerServerEvent('Polar-Paleto:Server:RemoveItem', drillitem, 1) end
        TriggerServerEvent('Polar-Paleto:Server:TargetRemove', door) 
    else
        SetPedComponentVariation(PlayerPedId(), 5, Config.BagUseID, 0, 1)
        TriggerServerEvent('Polar-Paleto:Server:StartInteract', door)
...
function starttarget()
    ...
    if oxt then
        ...
        options = {{  name = paletostartname, icon = "fas fa-fire", label = "Thermite", event = 'Polar-Paleto:client:start', canInteract = function(_, distance) return distance <= Config.OxTargetDistance end }, } })
    else
    ...
    { options = {{ event = "Polar-Paleto:client:start", icon = "fas fa-fire", label = "Thermite"}}, distance = 1 }) 
...
RegisterNetEvent('Polar-Paleto:Client:StartLoot', function()
    ...
    TriggerServerEvent('Polar-Paleto:Server:SetupPainting1')

    TriggerServerEvent('Polar-Paleto:Server:SetupCase1')
...
RegisterNetEvent('Polar-Paleto:Client:StartTargets', function()
    ...
    CreateTarget(paletodoor1name, vec3(Config.Door1Eye.x, Config.Door1Eye.y, Config.Door1Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)
    CreateTarget(paletodoor2name, vec3(Config.Door2Eye.x, Config.Door2Eye.y, Config.Door2Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)
    CreateTarget(paletodoor3name, vec3(Config.Door3Eye.x, Config.Door3Eye.y, Config.Door3Eye.z + 0.2), "Polar-Paleto:Client:Door", "Thermite", "fas fa-fire", Config.Debug)

    CreateTarget(paletodoorcard1name, vec3(Config.doorcard1Eye.x, Config.doorcard1Eye.y, Config.doorcard1Eye.z + 0.2), "Polar-Paleto:Client:doorcard", "Insert Card", "fas fa-bolt", Config.Debug)
    CreateTarget(paletodoorcard2name, vec3(Config.doorcard2Eye.x, Config.doorcard2Eye.y, Config.doorcard2Eye.z + 0.2), "Polar-Paleto:Client:doorcard", "Insert Card", "fas fa-bolt", Config.Debug)
...
--- Thermites
RegisterNetEvent('Polar-Paleto:Client:Door', function(data)
    local name = data.id
    door = name
    if name == paletodoor1name then
    ...
    elseif name == paletodoor2name then
    ...
    elseif name == paletodoor3name then
    ...
    callback('Polar-Paleto:Door' .. name, function(result) if result then 
        TriggerEvent('Polar-Paleto:client:Thermite', pp, door, coords) 
    else  notify(text('sometingelse'), "error") end end)
end)

----- CARD INSERT
RegisterNetEvent('Polar-Paleto:Client:doorcard', function(data) 
    item = carditem  
    rot = vector3(0.0, 0.0, 37.0) 
    door = data.id
    if door == paletodoorcard1name then
        position = Config.doorcard1swipe 
    elseif door == paletodoorcard2name then
        position = Config.doorcard2swipe 
    end
    TriggerEvent('Polar-Paleto:client:keycard', door, position, rot, item) 
end)
...
RegisterNetEvent('Polar-Paleto:Client:PaletoProp', function(door, prop, var, value, number) 
    loadModel(prop) 
    if value then
        if number == 1 then
...
RegisterNetEvent('Polar-Paleto:Client:ResetDoors', function()
    if oxd then
        doorlock(paletodoor1name, 1)
        doorlock(paletodoor2name, 1)
        doorlock(paletodoor3name, 1)
        doorlock(paletodoorcard1name, 1)
        doorlock(paletodoorcard2name, 1)
        doorlock(Config.VaultDoorName, 1)
...
function clear(v, pos)
    ...
    if Config.Lockdown then TriggerServerEvent('Polar-Paleto:Server:LockDown') end
...
RegisterNetEvent('Polar-Paleto:Client:StopLazers', function(data)
    disabled = false
    TriggerServerEvent('Polar-Paleto:Server:TargetRemove', data.id) 
...
function hacking(door, loc)
    ...
    NetworkAddEntityToSynchronisedScene(usb, netScene3, animDict, 'success_react_exit_var_01_ch_prop_ch_usb_drive01x', 4.0, -8.0, 1) Wait(200) 
    NetworkStartSynchronisedScene(netScene) Wait(4000) NetworkStartSynchronisedScene(netScene2) Wait(2000)
...
RegisterNetEvent('Polar-Paleto:Client:PaintTarget', function(data)
    local door = data.type
...
function snatch(name, item, chances)
    TriggerServerEvent('Polar-Paleto:Server:TargetRemove', name)
    ...
    local bagEntity =  CreateObject("hei_p_m_bag_var22_arm_s",  vec3(offset.x, offset.y, offset.z - 4.0), 1, 1, 0) 
    ...
    TriggerServerEvent('Polar-Paleto:Server:Synapse', name) 
